describe("Animate.js", function () {
    ...
    describe("initWhenCreate", function () {
        ...
        it("获得精灵的一个动画", function () {
            animate.initWhenCreate(fakeAnimation);

            expect(animate.ye___anim).toEqual(fakeAnimation);
        });
        it("获得当前播放动画的所有帧", function () {
//            sandbox.spy(fakeAnimation, "getFrames");

            animate.initWhenCreate(fakeAnimation);

            expect(fakeAnimation.getFrames.called).toBeTruthy();
        });
        it("获得每帧持续时间", function () {
            sandbox.spy(fakeAnimation, "getDurationPerFrame");

            animate.initWhenCreate(fakeAnimation);

            expect(fakeAnimation.getDurationPerFrame.calledOnce).toBeTruthy();
        });
        ...
        it("设置每帧的序号", function () {
            ...
            expect(animate.ye___anim.setFrameIndex.calledWith(fakeFrames)).toBeTruthy();
        });
    ...
    describe("update", function () {
        ...
        describe("否则", function () {
            ...
            it("如果最后一帧播放完毕，则结束动作并返回", function () {
                ...
                expect(animate.finish.calledOnce).toBeTruthy();
            ...
            it("否则，播放下一帧", function () {
                ...
                expect(animate.ye___setCurrentFrame.calledWith(2)).toBeTruthy();
            });
        });
        ...
        it("将动画数据animSize保存到当前帧中", function () {
            ...
            expect(animate.ye___currentFrame.setCacheData.calledWith("animSize", animSize)).toBeTruthy();
        });
        it("设置当前帧为target的播放帧", function () {
            animate.update();

            expect(fakeTarget.setDisplayFrame.calledWith(animate.ye___currentFrame)).toBeTruthy();
        });
    });
    ...
    describe("ye___setCurrentFrame", function () {
        ...
        it("设置序号对应的当前帧", function () {
            ...
            expect(animate.ye___frames.getChildAt.calledWith(2)).toBeTruthy();
        });
...
//        });
        it("重置播放的动画", function () {
            ...
            expect(animate.ye___setCurrentFrame.calledWith(0)).toBeTruthy();
        });
    });
...
//            sandbox.stub(animate, "getTarget").returns(fakeTarget);
        ...
        it("刷新每帧的序号", function () {
            ...
            expect(fakeAnimation.setFrameIndex.calledWith(frames)).toBeTruthy();
        });
        it("当前帧反序", function () {
            animate.reset();
...
//            animate.reverse();
//
//            expect(fakeTarget.setDisplayFrame.calledWith(fakeFrame2)).toBeTruthy();
//        });
    });
