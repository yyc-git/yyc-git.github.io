...
//    });
    ...
    describe("解决发现的问题", function () {
        ...
        describe("使用baseClass访问父类成员", function () {
            describe("解决“若A1为抽象类，A2（抽象类）继承于A1，B（类）继承于A2，A1、A2、B都有同名方法a，A2和B在a方法中都通过this.baseClass调用父类同名方法。则如果B的实例b调用a方法，则A2、B的a方法中的this.baseClass均指向A2（照理说A2的this.baseClass应该指向A1）！", function () {
                it("可以使用baseClass属性来访问父类成员，并通过加上不同的前缀，访问对应级别的父类：" +
                    "如baseClass如父类成员，_baseClass调用上一级父类成员，依次类推", function () {
                    var A1 = YYC.AClass({
                        Public: {
                            arr: [],
                            a: function () {
            ...
            it("不能使用getBaseClass(index)方法来访问父类，" +
                "因为当子类使用getBaseClass调用的父类成员中也使用getBaseClass调用上级父类成员时，会出现问题", function () {
                ...
                //如index为0或没有参数则访问上一级父类，index为1则访问上上一级父类，依次类推
                function _getBaseClass(index, self) {
                ...
                var A2 = YYC.AClass(A1, {
                    Public: {
                        a: function () {
                            this.arr.push(2);
                            //当调用B->a时，会调用A2->a，从而执行该代码。
                            //期望该代码调用A1->a，实际上却是调用A2->a！
                            //这是因为此时this指向B的实例，而不是指向A2！
                            //所以，_getBaseClass(0, this)返回baseClass，该baseClass指向A2！
                            _getBaseClass(0, this).a.call(this, null);
                        }
                    }
                });
                var B = YYC.Class(A2, {
                    Public: {
                        a: function () {
                            this.arr.push(3);
                            _getBaseClass(0, this).a.call(this, null);
                            _getBaseClass(1, this).a.call(this, null);
