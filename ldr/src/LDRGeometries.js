...
LDR.LDRGeometry = function () {
    ...
    this.boundingBox = adapter.BoundingBox3.create();
}
...
LDR.LDRGeometry.prototype.buildGeometriesAndColorsForLines = function () {
    ...
    if (allLineColors.length === 1) {
        ...
        lineVertexAttribute = adapter.Geometry.createFloat32BufferAttribute(lineVertices, 3);
        ...
        lineVertexAttribute = adapter.Geometry.createFloat32BufferAttribute(lineVertices, 4);
    }
...
LDR.LDRGeometry.prototype.buildGeometriesAndColors = function () {
    ...
    allTriangleColors.forEach(c => {
        ...
        let triangleVertexAttribute = adapter.Geometry.createFloat32BufferAttribute(triangleVertices, 3);
...
LDR.LDRGeometry.prototype.buildGeometry = function (indices, vertexAttribute) {
    ...
    let g = adapter.Geometry.createBufferGeometry();
    g = adapter.Geometry.setIndex(indices, g);
    g = adapter.Geometry.setAttribute('position', vertexAttribute, g);
...
LDR.LDRGeometry.prototype.buildGeometryForConditionalLines = function (multiColored, conditionalLines) {
    ...
    this.conditionalLineGeometry = adapter.Geometry.createBufferGeometry();
    ...
    this.conditionalLineGeometry = adapter.Geometry.setAttribute(
        'position', adapter.Geometry.createFloat32BufferAttribute(p1s, 3), this.conditionalLineGeometry
    );
    this.conditionalLineGeometry = adapter.Geometry.setAttribute(
        'p2', adapter.Geometry.createFloat32BufferAttribute(p2s, 3),

        this.conditionalLineGeometry
    );
    this.conditionalLineGeometry = adapter.Geometry.setAttribute(
        'p3', adapter.Geometry.createFloat32BufferAttribute(p3s, 3),

        this.conditionalLineGeometry
    );
    this.conditionalLineGeometry = adapter.Geometry.setAttribute(
        'p4', adapter.Geometry.createFloat32BufferAttribute(p4s, 3),

        this.conditionalLineGeometry
    );
    if (multiColored) {
        this.conditionalLineGeometry = adapter.Geometry.setAttribute(
            'colorIndex', adapter.Geometry.createFloat32BufferAttribute(new Float32Array(colorIndices), 1),
...
LDR.LDRGeometry.prototype.fromPartDescription = function (loader, pd) {
    ...
    let m3e = adapter.Matrix3.getElements(pd.r);
    let m4 = adapter.Matrix4.create(m3e[0], m3e[3], m3e[6], pd.p.x,
        m3e[1], m3e[4], m3e[7], pd.p.y,
        m3e[2], m3e[5], m3e[8], pd.p.z,
        0, 0, 0, 1
    );
    this.boundingBox = adapter.BoundingBox3.applyMatrix4(m4, this.boundingBox);
    ...
    let p = adapter.Vector3.create(0, 0, 0);
    let lp = pd.logoPosition;
    for (let i = 0; i < this.vertices.length; i++) {
        let v = this.vertices[i];
        v.oldIndex = i;

        p = adapter.Vector3.set(v.x, v.y, v.z, p);
        p = adapter.Vector3.applyMatrix3(pd.r, p);
        p = adapter.Vector3.add(pd.p, p);
