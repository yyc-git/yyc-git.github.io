...
var MapEditor = {
		...
		this.menu = new DOTA.MENU.Menu({id : "mainMenu", items : [
			{
					...
					items : [
						{
							...
							text : '保存', handler : DOTA.Event.bindEvent(this, this.handler, 'Save')
						}, '-' , {
							text : '退出', handler : DOTA.Event.bindEvent(this, this.handler, 'Exit')
						}
					]
				}
			}, {
				...
				menu : {
					items : [
						{ text : '清空' , handler : DOTA.Event.bindEvent(this, this.handler, 'Clear') },
						{ text : '全选' , handler : DOTA.Event.bindEvent(this, this.handler, 'SelectAll') }
					]
				}
			}, {
				text : '帮助(H)',
				menu : {
					items : [
						 { text : '关于', handler : DOTA.Event.bindEvent(this, this.handler, 'About') }
					]
				}
			}
		]});
	...
	initEvent : function(){
		...
		DOTA.Event.addEvent(document, "contextmenu", DOTA.Event.bindEvent(this, this.onContext));   //右击，需要修改
	},
	init : function(){
		this.mouseDown = true;
		this.flag = false;
