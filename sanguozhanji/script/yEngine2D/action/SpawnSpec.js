    ...
    describe("update", function () {
        ...
        describe("判断是否完成动作", function () {
            ...
            it("否则，不调用finish", function () {
                ...
                expect(action.finish.called).toBeFalsy();
            });
        });
        ...
        it("依次调用每个未完成的动作的update", function () {
            ...
            expect(action1.update.called).toBeFalsy();
            expect(action2.update.calledOnce).toBeTruthy();
            expect(action3.update.calledOnce).toBeTruthy();
            expect(action2.update.calledBefore(action3)).toBeTruthy();
        });
    });
...
//        action2.isFinish = sandbox.stub().returns(false);
    ...
    describe("调用所有内部动作的方法", function () {
        function judgeIterator(method) {
            ...
            expect(action1[method].calledOnce).toBeTruthy();
            expect(action2[method].calledOnce).toBeTruthy();
            expect(action1[method].calledBefore(action2[method])).toBeTruthy();
        }
        ...
        function judgeIteratorWithArg(method, arg) {
            ...
            expect(action1[method].calledWith(arg)).toBeTruthy();
            expect(action2[method].calledWith(arg)).toBeTruthy();
        }
    ...
    describe("reverse", function () {
        it("将动作序列反转，并反转每个动作，返回反转后的动作", function () {
            ...
            expect(action1.reverse.calledOnce).toBeTruthy();
            expect(action2.reverse.calledOnce).toBeTruthy();
            expect(action1.reverse.calledAfter(action2.reverse)).toBeTruthy();
...
//        });
        ...
        it("返回动作副本，拷贝内部动作", function () {
            ...
            expect(YE.Spawn.create.calledWith(copyAction1, copyAction2)).toBeTruthy();
        });
    });
