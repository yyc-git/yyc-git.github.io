...
YYC.Paging = MyClass({
    ...
    Private: {
        ...
        //设置宽度
        _SetWidth: function (rowCount, width) {
            //            var totalWidth = (width + 7) * rowCount;    //总宽度，5为每项间的间距
            ...
            var ulWidth = (width + 15) * rowCount;    //list的总宽度，15为每项间的间距的估计值
            var divWidth = 0;
            ...
            */
            setTimeout(function () {
                ...
                temp = operate.GetMaxWidthHeight(self._li); //获得li中最大的width和height
        ...
        _Validate: function () {
            ...
            for (i = 0, len = this.config.items.length; i < len; i++) {
                if (this.config.onlyImg || this.config.onlyText) {
                    if (MyGameEngine.Base.IsArray(this.config.items[i])) {
            ...
            if (!operate.IsString(this.config.imgBorder)) {
            ...
            if (jtext.length > 0) {
                ...
                if (!regex.test(jtext) || jtext === "0") {  //页号为0也不行
                    ...
                    if (jtext == pageIndex) {     //输入的页码为当前页
                        ...
                        //                        MyGameEngine.Event.BindEvent(this, this._GoPage, jtext)(window.event);
        ...
        _AddButton: function () {
            ...
            this._but = new YYC.Button({
                ...
                //                }
                onClick: MyGameEngine.Event.BindEvent(self, self._Jump, [self.pageNum, self.pageIndex])  //调用Jump方法
            });
        ...
        _InitHtml: function () {
            ...
            //只有文字
                ...
                str = MyGameEngine.Base.FormatDelegate("<li><span style='display:block;width:{0}px;'></span></li>", this.config.width);
            }
            //有图片和文字
            else {
            ...
            for (var i = 0; i < li.length; i++) {
                ...
                (function (i) {
                    ...
                    MyGameEngine.Event.AddEvent(li[i], "click", MyGameEngine.Event.BindEvent(self, self._OnClick, i, li[i]));
                })(i);
                //                    //显示当前选中的地图
                //                    index = i + (self.pageIndex - 1) * self.pageSize;
        ...
        _ShowPage: function () {
            ...
            //获得函数名
            var _GoPage = MyGameEngine.Base.IsFunction(this.config._GoPage) ? MyGameEngine.Base.GetFunctionName(this.config._GoPage) : this.config._GoPage;
            var temp = null;
            var handler = null;
            var element = null;
            var evt = MyGameEngine.Event;

            //            str = MyGameEngine.Base.FormatDelegate("<span>共<b>{0}</b>个 每页显示<b>{1}</b>个 第<b>{2}</b>/<b>{3}</b>页</span>", this.total, this.pageSize, this.pageIndex, this.pageNum);
            str = MyGameEngine.Base.FormatDelegate("<span>共<b>{0}</b>个 第<b>{1}</b>/<b>{2}</b>页</span>", this.total, this.pageIndex, this.pageNum);
            ...
            if (this.pageIndex > (show + 1)) {
                str = MyGameEngine.Base.FormatDelegate("<a name='before' title='前" + (show + 1) + "页'>{0}</a>", "..");
            ...
            for (var i = this.pageIndex - show; i <= this.pageIndex + show; i++) {
                if (i == this.pageIndex) {
                    str = MyGameEngine.Base.FormatDelegate("<a onclick='void(0);' class='current'>{0}</a>", i);

                    temp.append($(str));
                }
                else {
                    if (i > 0 & i <= this.pageNum) {
                        str = MyGameEngine.Base.FormatDelegate("<a name='page' >{0}</a>", i);
            ...
            if (this.pageIndex < (this.pageNum - (show))) {
                str = MyGameEngine.Base.FormatDelegate("<a name='after' title='后" + (show + 1) + "页' >{0}</a>", "..");
            ...
            //                //                    html.push("<a onclick='void(0);' class='gray'>上一页</a>");
            //            } else {
            //                str = MyGameEngine.Base.FormatDelegate("<a name='prev' >上一页</a>");
            //                //                    html.push("<a onclick=\"goPage('prev');void(0);\">上一页</a>");
            //            }
            ...
            //            if (this.pageIndex > (show + 1)) {
            //                str += MyGameEngine.Base.FormatDelegate("<a name='before' title='前" + (show + 1) + "页' onclick='{0}({1})'>{2}</a>", _GoPage, this.pageIndex - (show + 1), "..");
            //            }
            //            for (var i = this.pageIndex - show; i <= this.pageIndex + show; i++) {
            //                if (i == this.pageIndex) {
            //                    str += MyGameEngine.Base.FormatDelegate("<a onclick='void(0);' class='current'>{0}</a>", i);
            ...
            //                else {
            //                    if (i > 0 & i <= this.pageNum) {
            //                        str += MyGameEngine.Base.FormatDelegate("<a name='page' onclick='{0}({1});void(0);'>{2}</a>", _GoPage, i, i);
            //                    }
            //                }
            //            }
            //            if (this.pageIndex < (this.pageNum - (show))) {
            //                str += MyGameEngine.Base.FormatDelegate("<a name='after' title='后" + (show + 1) + "页' onclick='{0}({1})'>{2}</a>", _GoPage, this.pageIndex + (show + 1), "..");
            //            }
            ...
            //            //                } else {
            //            //                    str += MyGameEngine.Base.FormatDelegate("<a onclick='{0}({1});void(0);'>{2}</a>", _GoPage, i, i);
            ...
            //            } else {
            //                //                    html.push("<a onclick=\"goPage('next');void(0);\">下一页</a>");
            //                str += MyGameEngine.Base.FormatDelegate("<a name='next' onclick=\"{0}('next');void(0);\">下一页</a>", _GoPage);
            //            }


            //            temp = "<span id='paging_jump'>&nbsp;跳转到&nbsp;<input type='text' id='paging_jump_text' size='4' class='jump'/></span>";
            //            str += MyGameEngine.Base.FormatDelegate("<span id='paging_jump'>&nbsp;跳转到&nbsp;<input type='text' id='paging_jump_text' size='4' class='jump'/></span>");
    ...
    Public: {
        ...
        RenderTo: function (container) {
            ...
            //            //设置样式
            //            this._SetWidth("yyc_paging", "list", "paging", this.config.rowCount, this.config.width);
        ...
        Dispose: function () {
            ...
            MyGameEngine.Event.RemoveEvent(this._goPage, "click", this._OnClick_GoPage);
